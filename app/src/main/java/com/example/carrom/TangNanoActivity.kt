...
class TangNanoActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val txtStatus = findViewById<TextView>(R.id.txtTang)
        ...
        try {
            ...
            //Se envía 'T' para alternar (puedes adaptar VHDL para 'T' si lo deseas).
            toggleSensor.setOnClickListener {
                sensorState = !sensorState
                val comando = if (sensorState) "E" else "A"   // ejemplo: E = enable, A = disable
                sendCommand(comando)
            ...
            // Joystick: ahora se envían comandos  compatibles con el VHDL:
            // - 'S' -> stop
            // - 'U7' -> subir con pwm ~70% (7 -> 70)
            joystick.setOnMoveListener({ angle, strength ->
                // strength: 0..100 => map a 0..9
                val digit = ((strength / 10.0).toInt()).coerceIn(0, 9) // 0..9
                val comando = when {
                    strength == 0 -> "S"         // STOP
                    angle in 45..135 -> "U$digit"
                    angle in 135..225 -> "L$digit"
                    angle in 225..315 -> "D$digit"
                    else -> "R$digit"
                }
                sendCommand(comando)
            }, 200)
    ...
    private fun sendCommand(command: String) {
        try {
            ...
            // enviamos bytes ASCII, sin '\n' (vhdl procesará cada byte por separado)
